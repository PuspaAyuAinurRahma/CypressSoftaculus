...
use BeonOrder\Libs\Parser\Yaml\Exception\DumpException;
        ...
        while ($i < $len) {
            ...
            if (!$isKeyQuoted) {
                ...
                if ('' !== $key && $evaluatedKey !== $key && !is_string($evaluatedKey) && !is_int($evaluatedKey)) {
                    @trigger_error(self::getDeprecationMessage('Implicit casting of incompatible mapping keys to strings is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys instead.'), E_USER_DEPRECATED);
                }
            }
                ...
                switch ($mapping[$i]) {
                    case '[':
                        ...
                        if ('<<' === $key) {
                            ...
                            @trigger_error(self::getDeprecationMessage(sprintf('Duplicate key "%s" detected whilst parsing YAML. Silent handling of duplicate mapping keys in YAML is deprecated since Symfony 3.2 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0.', $key)), E_USER_DEPRECATED);
                        }
                        break;
                    case '{':
                        // nested mapping
