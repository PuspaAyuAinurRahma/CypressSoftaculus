...
namespace BeonOrder\Libs\Parser\Yaml;
        ...
        do {
            ...
            if (self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if (isset($values['value'][1]) && '?' === $values['value'][0] && ' ' === $values['value'][1]) {
                    @trigger_error($this->getDeprecationMessage('Starting an unquoted string with a question mark followed by a space is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0.'), E_USER_DEPRECATED);
                }
                ...
                if (!is_string($key) && !is_int($key)) {
                    ...
                    @trigger_error($this->getDeprecationMessage(sprintf('Implicit casting of %s to string is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys instead.', $keyType)), E_USER_DEPRECATED);
                }
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ($allowOverwrite || !isset($data[$key])) {
                            ...
                            @trigger_error($this->getDeprecationMessage(sprintf('Duplicate key "%s" detected whilst parsing YAML. Silent handling of duplicate mapping keys in YAML is deprecated since Symfony 3.2 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0.', $key)), E_USER_DEPRECATED);
                        }
                    } else {
