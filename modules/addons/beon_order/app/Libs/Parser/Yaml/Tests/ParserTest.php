...
namespace BeonOrder\Libs\Parser\Yaml\Tests;

use PHPUnit\Framework\TestCase;
use BeonOrder\Libs\Parser\Yaml\Exception\ParseException;
use BeonOrder\Libs\Parser\Yaml\Yaml;
use BeonOrder\Libs\Parser\Yaml\Parser;
use BeonOrder\Libs\Parser\Yaml\Tag\TaggedValue;
...
EOF;
    ...
    {
        $input = <<<'EOF'
foo: !php/object O:30:"BeonOrder\Libs\Parser\Yaml\Tests\B":1:{s:1:"b";s:3:"foo";}
bar: 1
EOF;
...
YAML;
    ...
    /**
     * @dataProvider invalidDumpedObjectProvider
     * @expectedException \BeonOrder\Libs\Parser\Yaml\Exception\ParseException
     */
    ...
    public function testNonUtf8Exception()
    {
        ...
        foreach ($yamls as $yaml) {
            try {
                ...
                $this->assertInstanceOf('BeonOrder\Libs\Parser\Yaml\Exception\ParseException', $e, 'charsets other than UTF-8 are rejected.');
            }
        }
    }
...
EOD;
     ...
     * @expectedDeprecation Duplicate key "%s" detected whilst parsing YAML. Silent handling of duplicate mapping keys in YAML is deprecated %s and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0 on line %d.
     * throws \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0
     */
...
EOF;
    ...
    /**
     * @group legacy
     * @expectedDeprecation Implicit casting of numeric key to string is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys instead on line 2.
     */
...
EOF;
    ...
    /**
     * @group legacy
     * @expectedDeprecation Implicit casting of non-string key to string is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys instead on line 1.
     */
...
EOT;
    ...
    {
        if (method_exists($this, 'expectException')) {
            $this->expectException('\BeonOrder\Libs\Parser\Yaml\Exception\ParseException');
            $this->expectExceptionMessage(sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        } else {
            $this->setExpectedException('\BeonOrder\Libs\Parser\Yaml\Exception\ParseException', sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        }
...
YAML
    ...
    /**
     * @group legacy
     * @expectedDeprecation Starting an unquoted string with a question mark followed by a space is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0 on line 1.
     */
...
YAML;
    ...
    /**
     * @group legacy
     * @expectedDeprecation Starting an unquoted string with a question mark followed by a space is deprecated since Symfony 3.3 and will throw \BeonOrder\Libs\Parser\Yaml\Exception\ParseException in 4.0 on line 2.
     */
...
YAML;
    ...
    public function testPhpConstantTagMappingKey()
    {
        $yaml = <<<YAML
transitions:
    !php/const 'BeonOrder\Libs\Parser\Yaml\Tests\B::FOO':
        from:
            - !php/const 'BeonOrder\Libs\Parser\Yaml\Tests\B::BAR'
        to: !php/const 'BeonOrder\Libs\Parser\Yaml\Tests\B::BAZ'
YAML;
        $expected = array(
            'transitions' => array(
                'foo' => array(
